...
public checkVotes()
{
	    ...
	    && g_voteCount[SELECTMAPS] > g_voteCount[SELECTMAPS+1])
	{
		...
		color_print(0, "^4[ NovusLink ]^1 Choosing finished. Current map will be extended to next ^3%.0f^1 minutes", steptime)
		client_cmd( 0, "spk gman/gman_noregret.wav" );
	...
	color_print(0, "^4[ NovusLink ]^1 Choosing finished. The nextmap will be^3 %s", smap)
	client_cmd( 0, "spk gman/gman_wise.wav" );
...
public countVote(id, key)
{
	if (get_cvar_float("amx_vote_answers"))
	{
		...
		if (key == SELECTMAPS)
			color_print(0, "^4[ NovusLink ]^3 %s^1 chose ^4map extending", name)
		else if (key < SELECTMAPS)
		{
			new map[32];
			ArrayGetString(g_mapName, g_nextName[key], map, charsof(map));
			color_print(0, "^4[ NovusLink ]^3 %s ^1chose^4 %s", name, map);
		}
	}
	++g_voteCount[key]
...
public voteNextmap()
{
	...
	new pos = format(menu, 511, g_coloredMenus ? "\d%L:\w^n^n" : "%L:^n^n", LANG_SERVER, "CHOOSE_NEXTM")
	...
	{
		pos += format(menu[pos], 511, "\r%d. \wExtend map \y%s^n", SELECTMAPS + 1, mapname)
		mkeys |= (1<<SELECTMAPS)
	}
	...
	color_print(0, "^4[ NovusLink ]^1 %L", LANG_SERVER, "TIME_CHOOSE")
